...
const murmurationField = (
  ...
  for (let i = 0; i < from.length; i++) {
    const cx = lerp(from[i][0], to[i][0], eased) +
      0.025 * Math.sin(t * 0.12 + i * 1.7) + 0.04 * (mx - 0.5)
    const cy = lerp(from[i][1], to[i][1], eased) +
      0.018 * Math.cos(t * 0.09 + i * 2.3) + 0.03 * (my - 0.5)
    ...
    const wobble =
      0.15 * Math.sin(dx * 12 + dy * 9 + t * 0.35 + i * 2.1) +
      0.07 * Math.cos(dx * 7 - dy * 15 + t * 0.25 + i * 3.3)
    const sig = br * (1 + wobble) * 0.55
    const sig2 = sig * sig
    if (sig2 > 0.0001) {
      ...
      setTimeout(() => {
        ...
        const buildShape = (name: string): Point[] => {
          ...
          if (name === "text" || name === "manifesto") {
            ...
            const row = Math.max(1, Math.floor((rows - lineCount) / 2))
            return buildParagraphPoints(
              text,
              col,
              row,
              width,

...
const murmurationField = (
  ...
  for (let i = 0; i < from.length; i++) {
    const cx = lerp(from[i][0], to[i][0], eased) +
      0.07 * Math.sin(t * 0.09 + i * 1.7) +
      0.04 * Math.cos(t * 0.14 + i * 3.9) +
      0.05 * (mx - 0.5)
    const cy = lerp(from[i][1], to[i][1], eased) +
      0.05 * Math.cos(t * 0.07 + i * 2.3) +
      0.03 * Math.sin(t * 0.12 + i * 4.1) +
      0.04 * (my - 0.5)
    ...
    const wobble =
      0.22 * Math.sin(dx * 14 + dy * 11 + t * 0.45 + i * 2.1) +
      0.12 * Math.cos(dx * 8 - dy * 18 + t * 0.3 + i * 3.3) +
      0.06 * Math.sin(dx * 22 + dy * 5 + t * 0.6 + i * 5.7)
